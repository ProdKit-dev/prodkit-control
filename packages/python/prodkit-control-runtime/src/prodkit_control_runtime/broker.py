    ...
    async def execute(
        ...
        *,
        ...
        if not claimed:
            ...
            existing_executor = self._executors.get(action.executor)
            observation = await existing_executor.observe(action, existing)
            verification = await self._verifier.verify(action, existing, observation)
